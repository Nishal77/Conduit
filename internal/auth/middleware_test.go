...
func TestMiddleware_RejectsGarbageJWT(t *testing.T) {
	handler, _ := newTestMiddleware(t)
...
func TestMiddleware_ValidJWTSetsTenantContext(t *testing.T) {
	redisClient := newTestRedis(t)
	keyValidator := NewAPIKeyValidator(redisClient, nil, time.Minute)
	jwtValidator := NewJWTValidator("test-secret-at-least-32-characters!", "https://conduit")

	const wantTenantID = "33333333-3333-3333-3333-333333333333"
	token, err := jwtValidator.IssueAccessToken(wantTenantID, "agent-1", []string{"mcp:call"}, time.Hour)
	...
	assert.Equal(t, "jwt", capture.authMethod)
}
